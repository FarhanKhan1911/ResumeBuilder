...
def education(request):
    if request.method == 'POST':
        ...
        edu = Education( collagename=collagename,  university=university, branch=branch, yearofpass=yearofpass, programinglang=programinglang, cgpa=cgpa, user=request.user)
        edu.save()
        return redirect('index')
    form = EducationForm()
...
def personaldetails(request):
    if request.method == 'POST':
        ...
        pd = Personaldetails(nationality=nationality, address=address, fathername=fathername, dob=dob, gender=gender, user=request.user)
        pd.save()
        return redirect('index')
